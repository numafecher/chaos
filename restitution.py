...
def plus01ampCallback(event):
    ampHandle.set_val(ampHandle.val+0.1)
...
freqax.cla()
freqHandle=w.Slider(freqax, 'freq', 0, 2000, valinit = 500)
...
while switchon == True:
    ...
    axes2.cla()  
    axes2.plot(tstamps[2000:3000:2],vb[2000:3000:2])
    axes2.set_xlabel('x')
    axes2.set_ylabel('y')
    axes2.relim()
    axes2.autoscale()
    ...
    axes1.cla()
    axes1.plot(vb[200:-300:2],vc[200:-300:2],'.')
    axes1.set_xlabel('Height')
    axes1.set_ylabel('Velocity')
    axes1.relim()
    axes1.autoscale()
